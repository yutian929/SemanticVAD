...
class EMAAveragingFunction:
    ...
    @torch.no_grad()
    def __call__(
        self, ema_param: Tensor, current_param: Tensor, num_averaged: Tensor
    ) -> Tensor:
...
def test_weight_averaging_deepcopy(tmp_path):
    ...
    class TestCallback(WeightAveraging):
        ...
        def setup(self, trainer, pl_module, stage) -> None:
            ...
            assert (
                self._average_model.module.train_dataloader
                is not pl_module.train_dataloader
            )
            assert (
                self._average_model.module.train_dataloader.__self__
                == self._average_model.module
            )
...
def test_ema(tmp_path, batch_norm: bool, iterable_dataset: bool):
    ...
    dataset = (
        RandomIterableDataset(32, 32) if iterable_dataset else RandomDataset(32, 32)
    )
...
def test_ema_accelerator(tmp_path, accelerator):
    ...
    _train(
        model, dataset, tmp_path, EMATestCallback(), accelerator=accelerator, devices=1
    )
...
def _train_and_resume(
    model: TestModel, dataset: Dataset, tmp_path: str, devices: int = 1, **kwargs
) -> TestModel:
    _train(
        model,
        dataset,
