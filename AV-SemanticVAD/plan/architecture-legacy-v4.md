...
> # ⚠️ 已归档 · 历史参照（2026-09-18 归档）
> **本文件是旧的 v4 架构（单说话人 · 冻结 X2-Turn · SoulX 范式 · 判逐帧 complete/incomplete），已被取代。**
> 现行权威架构见 **[`architecture.md`](architecture.md)**（多人 per-face · 端到端 · Route A 热启动骨干）。
> 保留本文件仅为：① 部分论证仍有效并被新架构继承（判别式读出优于生成式、80ms 前视免费、时间粒度不可退让、Kurata 视觉线索消融序、参数量-数据量耦合）；
> ② 追溯当初为何否决"端到端 Omni"（新 Route A 正是对那些否决理由的回应：复用流式音频骨干、保 80ms 粒度、判别式头，而非从零训 Omni）。
> **不要据本文件继续开发**——以 `architecture.md` 为准。
...
这是一个容易被忽略但有实质意义的区别（**不是**「冻结 Voxtral」）：
