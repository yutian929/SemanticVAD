...
| 顺序 | 组 | 计划单章节 | 性质 |
|---|---|---|---|
| 1 | 解锁 **V1 + V2** | **§P0.1** | **只需下 `config.json` + `tekken.json` 两个小文件，无需环境** |
| 2 | 环境与基线复现 **+ V3 + V7** | **§P0.2**（V3=P0.2.5，V7=P0.2.6） | V3/V7 **需模型已载入/能跑推理**，故在此而非第 1 步 |
| 3 | **探针 A / 探针 B** | **§P0.3 / §P0.4** | **两个 AUC 数字决定整篇论文的 framing** |

> ⚠️ **四个待核实项不能一起做**：V1/V2 是纯读配置，V3/V7 要模型在手。
> 混在一起会以为第一步就能全部解决。
